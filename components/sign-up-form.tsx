  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.auth.signUp({
        ...
        password,
        options: {
          emailRedirectTo: `${window.location.origin}/auth/confirm?next=${encodeURIComponent(`/auth/claim?next=${encodeURIComponent(next)}`)}`,
        },
      });
        ...
        <CardContent>
          <form onSubmit={handleSignUp}>
            ...
            <div className="mt-4 text-center text-sm">
              ...
              <Link href={`/auth/login?next=${encodeURIComponent(next)}`} className="underline underline-offset-4">
                Login
              </Link>
            </div>
          </form>
